...
| `order-api.yaml` | 1.2.1 | 创建/查询订单、模拟支付、取消订单（下单必填 `addressId`；订单含收货人快照与明细） |
...
- 2026-09-24：order 升级 1.2.1 —— 补录已实现但契约遗漏的 `POST /api/v1/orders/{orderNo}/complete`（C 端确认收货，SHIPPED→COMPLETED），修复 CODE_REVIEW v2 记录的契约漂移。接口行为无变化。
