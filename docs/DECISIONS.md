...
- [P0混沌测试] 决定：新增 scripts/chaos-test.sh——下单流量（随机支付/取消混合）中随机时机 docker kill order-service 并自动拉起，跑 3 轮后过宽限期（stale 窗口+2 轮扫描），以三条不变式断言：I1 库存台账守恒（Δstock == -ΔDEDUCTED 合计，T0 基线必须在制造流量前取）、I2 全表无孤儿 DEDUCTED（对应订单缺失或已取消）、I3 确认订单闭环（有去重行且 PENDING 对应 DEDUCTED）；手动执行不进 CI | 备选：精确故障注入（扣减成功后落单前暂停点）或进 CI | 理由：把「崩溃窗口靠 ERROR 日志暴露」的已知限制反转为可重复证明的自动收敛（2026-09-03 实测 3 杀 36 请求 14 单全过，第 1 轮 kill 砸中流量高峰时 10 个孤儿扣减全部被对账回补）；随机混沌比确定性注入覆盖面广且零代码侵入，共享 runner 上杀容器不稳定故不入 CI。首版曾误把 T0 基线快照放到测试后（I1 恒 0 空转），复查发现修正——测试本身也要被 review。
