...
> **历史存档声明（2026-09-24 加注）**：本文是项目开工时发给执行 Agent 的指令存档，**不再随代码更新**。文中版本锁定（JDK 17 / Spring Boot 3.2.4 / Spring Cloud 2023.0.1 / SCA 2023.0.1.0 / MyBatis-Plus 3.5.5 / Nacos v2.3.2）已被 `DECISIONS.md`「[v1.5升级]」决策整体取代，现行版本矩阵以 `AGENTS.md` §4.8 为权威、技术栈以 `README.md` 为准。**不要依据本文件回退任何版本号或环境假设。** 保留价值在于：执行步骤编排、gate 验收规则与决策日志格式。
